...
router.get('/all', async (req, res) => {
    try {
        const snapshot = await firebase_1.db.collection('profiles').get();
...
router.get('/mobile/:mobile', auth_1.verifyAuth, async (req, res) => {
    try {
...
router.get('/:profileId', auth_1.verifyAuth, async (req, res) => {
    try {
        const { profileId } = req.params;
        const profileDoc = await firebase_1.db.collection('profiles').doc(profileId).get();
...
router.post('/', auth_1.verifyAuth, async (req, res) => {
    try {
        ...
        console.log('Firestore collection reference:', typeof firebase_1.db);
        try {
            await firebase_1.db.collection('profiles').doc(profileId).set({
                ...newProfile,
                createdAt: admin.firestore.Timestamp.now(),
                mobile: profileWithDefaults.mobile,
                // Add geohash for location-based queries (for future optimization)
                geohash: encodeGeohash(parseFloat(profileWithDefaults.currentLocation?.lat || '0'), parseFloat(profileWithDefaults.currentLocation?.long || '0')),
            });
...
router.put('/:profileId', auth_1.verifyAuth, async (req, res) => {
    try {
        const { profileId } = req.params;
        const { profile } = req.body;
        if (!profile) {
        ...
        const profileRef = firebase_1.db.collection('profiles').doc(profileId);
        ...
        const updatedProfile = {
            ...
            geohash: encodeGeohash(parseFloat(profile.currentLocation?.lat || '0'), parseFloat(profile.currentLocation?.long || '0')),
        };
...
router.delete('/:profileId', auth_1.verifyAuth, async (req, res) => {
    try {
        const { profileId } = req.params;
        const profileRef = firebase_1.db.collection('profiles').doc(profileId);
        const profileSnapshot = await profileRef.get();
...
router.get('/place/:placeName', auth_1.verifyAuth, async (req, res) => {
    try {
...
router.get('/friends/:profileId', auth_1.verifyAuth, async (req, res) => {
    try {
        const { profileId } = req.params;
        const profileDoc = await firebase_1.db.collection('profiles').doc(profileId).get();
        if (!profileDoc.exists) {
...
router.post('/nearby', auth_1.verifyAuth, async (req, res) => {
    try {
...
router.post('/seed/dummy', async (req, res) => {
    try {
        ...
        for (let i = 0; i < dummyUsers.length; i++) {
            ...
            await firebase_1.db.collection('profiles').doc(userId).set(profileData);
            created++;
